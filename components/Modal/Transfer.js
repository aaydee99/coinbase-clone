...
const Transfer = ({selectedToken, walletAddress, thirdWebTokens, setAction}) => {
  ...
  const [activeThirdWebToken, setActiveThirdWebToken] = useState(thirdWebTokens[0]);
 ...
 const sendCrypto = async(amount, recipient) => {
     if(activeThirdWebToken && amount && recipient){
         await activeThirdWebToken.transfer(recipient, amount.toString().concat('000000000000000000'));
    ...
    <Wrapper>
        ...
        <TransferForm>
            <Row>
                ...
                <Recipient placeholder='Address' value={recipient} onClick={e => setRecipient(e.target.value)}/>
            </Row>
            <Divider/>
            <Row>
